...
namespace GUI
{
    public partial class frmSachBanChay : Form
    {
        ...
        private void LayDSSachBanChay()
        {
            try
            {
                lsvChiTiet.Items.Clear();
                DataTable dt = new DataTable();
                dt = clsSachBLL.Instance.LayDSSachBanChay(dtpFrmDate.Value, dtpToDate.Value);
                if (dt != null)
                {
                    int i = 1;
            ...
            try
            {
                using (frmBaoCao f = new frmBaoCao())
                {
                    try
                    {
                        DataTable dt = new DataTable();
                        dt = clsSachBLL.Instance.LayDSSachBanChay(dtpFrmDate.Value, dtpToDate.Value);
                        f.Text = "BÁO CÁO DANH SÁCH SÁCH BÁN CHẠY";
                        f.rptBaoCao.ProcessingMode = ProcessingMode.Local;
                        f.rptBaoCao.LocalReport.ReportPath = "Report\\rptSachBanChay.rdlc";
        ...
        private void frmSachBanChay_Load(object sender, EventArgs e)
        {
            DatNgay();
            LayDSSachBanChay();
        }
